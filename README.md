...
## 이번 업데이트 (v0.1.12) — 패널 배치·PDF·원격 OneDrive 개선

사무실 서버에 둔 사건을, 노트북에서 로컬과 똑같이 다룰 수 있습니다.

- **양방향 작업 패널** — 탐색기에서 연 문서는 왼쪽에, 터미널은 오른쪽에 기본으로 열립니다. 필요하면 문서를 오른쪽으로, 터미널을 왼쪽으로 옮겨 사건기록·작성서류·Claude를 한 화면에 배치할 수 있습니다.
- **문서/터미널 새 창 분리** — 문서를 끌어 새 창으로 열 때 시작 화면 없이 문서만 열리고, 터미널도 끌어서 별도 창으로 분리할 수 있습니다.
- **PDF/이미지 보기 개선** — 암호가 걸린 PDF는 열 때 비밀번호를 묻고, 확대 상태에서는 손 도구로 문서·이미지를 끌어가며 볼 수 있습니다. PDF도 패널/새 창 이동 대상입니다.
- **원격 OneDrive 자동 실체화** — SSH로 여는 원격 Mac의 OneDrive 파일이 클라우드 전용(`dataless`)이면, 앱이 원격 Mac에서 `rclone`으로 먼저 내려받아 OneDrive 자리표시자 파일을 실제 파일로 바꿉니다.
- **사건기록 PDF 사전 다운로드** — 원격 사건기록 폴더를 열어 PDF 목록을 만들 때 하위 PDF들을 원격 Mac에 순차적으로 내려받아, 이후 열람 때 OneDrive 클라우드 지연을 줄입니다.
- **rclone 한글 경로 안정화** — macOS OneDrive의 조합형 한글 경로를 rclone 클라우드 경로에 맞게 정규화해 `directory not found` 오류를 줄였습니다.
- **원격 파일 자동 새로고침** — 원격에서 Claude나 다른 편집기로 수정한 파일을 탐색기·뷰어·Markdown 편집기에 주기적으로 반영합니다. 로컬에서 미저장 편집 중인 내용은 덮어쓰지 않습니다.
- **원격 세션/동기화 보강** — 과거 Claude 세션을 원격에서도 다시 찾고, macOS Homebrew 경로의 rclone을 비대화형 SSH 환경에서도 찾도록 보강했습니다.
...
- **원격 OneDrive 최신화** — SSH 폴더 찾기에서 OneDrive 위치에 들어간 뒤 **OneDrive 최신화**로 클라우드의 최신 폴더/빈 사건 폴더까지 원격 Mac에 내려받고 다시 탐색.
