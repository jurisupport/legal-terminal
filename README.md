...
## 이번 업데이트 (v0.1.30) — Agent SDK 기반·소송기록 후보·원격 폴더 탐색

2026년 6월 4일 릴리즈는 "사건 폴더와 소송기록을 더 정확히 연결하고, Claude 작업 흐름을 구조화된 Agent 이벤트로 확장할 기반을 마련하는 것"에 초점을 맞췄습니다.

- **Claude Agent SDK 연결부 추가** — Claude 작업을 세션·메시지·도구 실행·권한 요청·diff 제안 이벤트로 받을 수 있는 IPC 기반을 추가했습니다. 기존 터미널형 Claude 사용은 그대로 유지됩니다.
- **소송기록 폴더 후보 추천** — 사건번호가 유일하게 맞으면 자동 연결하고, 애매하면 사건번호·사건명·당사자명 기준 후보를 여러 개 보여줘 사용자가 직접 선택할 수 있습니다.
- **원격 폴더 선택 개선** — 원격 폴더 선택창에서 현재 위치 아래 폴더명을 검색하고, 새 폴더를 바로 만들 수 있습니다.
- **소송기록 최신화 분리** — 기록뷰어에서 원격 소송기록 폴더를 클라우드에서 내려받는 최신화 흐름을 바로 열 수 있고, 이 경우 올리기 없이 내려받기만 수행합니다.
- **작업 알림 보강** — Claude 완료·질문 대기 상태가 백그라운드 창에서도 도크 바운스/플래시로 표시되어, 작업 중인 창을 빼앗지 않고 주의를 끕니다.
- **Markdown/PDF 출력 안정화** — Markdown 수평선을 프리뷰에서 실제 구분선처럼 표시하고, PDF 내보내기 저장 대화상자가 출력을 시작한 창에 붙도록 안정화했습니다.
- **macOS/Windows 배포 반영** — [v0.1.30 릴리스](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.30)는 macOS arm64/x64, Windows x64 빌드와 업데이트 메타데이터를 포함합니다.
