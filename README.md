...
## 이번 업데이트 (v0.1.39) — Agent 대기열 제어·프롬프트 히스토리·탭 단축키 보강

2026년 6월 5일 릴리스는 Agent Panel 입력 경험과 작업 중 지시 제어를 다듬는 데 초점을 맞췄습니다. 보낸 프롬프트를 방향키로 다시 불러오고, 작업 중에도 새 지시를 대기열에 넣거나 바로 실행하도록 승격할 수 있습니다.

- **Agent 프롬프트 히스토리** — Agent 입력창에서 `↑`를 누르면 이전에 보낸 프롬프트가 다시 나오고, `↓`로 앞으로 이동하거나 작성 중이던 초안으로 돌아올 수 있습니다.
- **세션 재개 히스토리 연결** — 이어서 연 Agent 세션은 기존 transcript의 사용자 프롬프트도 히스토리에 반영해, 과거 요청을 바로 다시 불러올 수 있습니다.
- **대기열 지시 제어** — 작업 중 새 입력은 대기열에 넣고, 대기 중인 지시는 카드에서 바로 지시하기로 승격하거나 삭제할 수 있습니다.
- **바로 지시하기 정리** — 바로 지시하기는 현재 작업과 권한/질문 대기를 정리한 뒤 새 지시를 우선 실행하도록 내부 흐름을 분리했습니다.
- **포커스 기준 탭 단축키** — `Ctrl/Cmd+W`, `Ctrl/Cmd+Tab`, `Ctrl/Cmd+PageUp/PageDown`이 현재 포커스된 Agent/터미널/문서 탭을 기준으로 동작하도록 보강했습니다.
- **Agent에서 새 터미널 열기 개선** — Agent Panel에 포커스가 있을 때 `Ctrl/Cmd+T` 또는 터미널 열기 버튼으로 같은 사건의 터미널을 정확한 패널 쪽에 엽니다.
- **파일·폴더 인라인 생성 보강** — 새 문서/새 폴더를 파일트리에서 바로 이름 붙여 만들 수 있고, 빈 폴더와 로딩 상태 표시가 더 명확해졌습니다.
- **원격 폴더 선택 안정화** — 원격 폴더 목록을 연속으로 불러올 때 늦게 도착한 이전 응답이 현재 화면을 덮어쓰지 않도록 막았습니다.
- **macOS/Windows 배포 반영** — [v0.1.39 릴리스](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.39)는 macOS arm64/x64, Windows x64 빌드와 업데이트 메타데이터를 포함합니다.
