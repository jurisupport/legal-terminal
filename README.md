...
## 이번 업데이트 (v0.1.36) — Agent Panel 기본화·과거 대화 복원·세션 미리 불러오기

2026년 6월 5일 릴리스는 "기존 터미널형 Claude를 필요할 때만 쓰고, 평소에는 VS Code의 Claude Code처럼 구조화된 Agent Panel에서 사건 작업을 진행하는 것"에 초점을 맞췄습니다. v0.1.36 패치에서는 작업환경을 열자마자 해당 사건 폴더의 과거 Claude 세션을 미리 불러와, 지난 대화 목록을 기다리지 않고 바로 이어서 볼 수 있도록 보강했습니다.

- **Agent Panel 기본화** — 사건을 열면 터미널 대신 Claude Agent 패널이 기본으로 열립니다. 터미널은 탭 메뉴의 **터미널로 실행**에서 그대로 사용할 수 있습니다.
- **지난 대화 화면 복원** — 과거 세션을 불러오면 Claude transcript에서 사용자 질문과 Claude 답변을 다시 읽어 Agent Panel에 표시합니다. 새 지시를 보내기 전에도 앞선 대화 흐름을 눈으로 확인할 수 있습니다.
- **작업환경 세션 미리 불러오기** — 저장해 둔 작업환경을 열면 열린 사건/터미널의 과거 Claude 세션 목록을 백그라운드에서 미리 조회합니다. 세션 목록을 눌렀을 때 이전 대화를 더 빠르게 찾을 수 있습니다.
- **정렬 순서 기억** — 파일 탐색기와 원격 폴더 선택 창에서 고른 정렬 기준을 저장해 다음 실행 때도 같은 순서로 보여줍니다.
...
- **진행 중 큐·스티어링** — Claude가 작업 중이어도 다음 지시를 큐에 넣거나, 현재 작업을 끊고 새 지시로 방향 전환할 수 있습니다.
...
- **macOS/Windows 배포 반영** — [v0.1.36 릴리스](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.36)는 macOS arm64/x64, Windows x64 빌드와 업데이트 메타데이터를 포함합니다.
