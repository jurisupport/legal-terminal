...
## 이번 업데이트 (v0.1.28) — 탭 재결합·드래그 안내·창 표시 안정화

2026년 6월 3일 릴리즈는 "작업 중인 문서와 터미널을 원하는 창/패널에 정확히 붙이고, 파일을 끌어다 놓을 때 결과를 미리 알 수 있게 하는 것"에 초점을 맞췄습니다.

- **찢어낸 탭 다시 붙이기** — 새 창으로 떼어낸 문서/터미널 탭을 기존 창의 탭바 위로 드롭해 다시 합칠 수 있습니다.
- **좌우 패널 간 탭 이동** — 문서뿐 아니라 터미널도 왼쪽/오른쪽 작업영역 사이에서 이동할 수 있습니다. 이동 버튼은 각 패널의 활성 탭 기준으로 동작합니다.
- **탭 드롭존 표시** — 탭을 끌고 있을 때 붙일 수 있는 탭바가 accent 색과 dashed 영역으로 강조되어, 어느 창/패널에 붙는지 보입니다.
- **파일 드롭 안내** — 외부 파일이나 앱 안의 파일을 끌 때, 작성서류 폴더에 복사되는지, 특정 폴더로 이동되는지, 터미널/Claude에 파일 질문으로 전달되는지 화면에 표시합니다.
- **창 표시 안정화** — 새 작업환경 창과 찢어낸 전용 창이 숨은 채 남지 않도록 `ready-to-show`, 로드 완료, fallback 타이밍에서 표시·포커스를 보강했습니다.
- **상태바/PDF 상태 표시 개선** — 현재 문서, PDF 쪽수·배율, 사건 연결, 소송기록 연결, 터미널/Claude 상태를 하단 상태바에서 더 잘 파악할 수 있게 했습니다.
- **macOS/Windows 배포 반영** — [v0.1.28 릴리스](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.28)는 macOS arm64/x64, Windows x64 빌드와 업데이트 메타데이터를 포함합니다.
...
- 탐색기: 파일트리, 인라인 새 파일/폴더, 드래그앤드롭 이동·복사(폴더 자동 펼침 + 드롭 결과 안내), **우클릭 삭제**(폴더 재귀), 클립보드 파일 붙여넣기, 원격 파일/폴더 다운로드
