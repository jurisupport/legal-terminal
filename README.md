...
최신 릴리스: [v0.1.127](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.127)

- **사건 열기 흐름 정리**: 사건이나 폴더를 열 때 빈 Agent/터미널 탭을 자동으로 만들지 않습니다.
- **명시 실행 유지**: 사용자가 `새 Agent` 또는 `터미널로 실행`을 누른 경우에만 작업 탭을 엽니다.
- **원격 파일 안정화**: 느린 클라우드 I/O에서 원격 파일 실체화와 단일 파일 동기화 흐름을 보강했습니다.
- **케이스탭 조작 개선**: Windows 단축키 충돌을 피하고 케이스탭 닫기 동작을 안정화했습니다.
