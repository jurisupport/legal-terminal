...
## 이번 업데이트 (v0.1.37) — Agent 파일 첨부·원격 OneDrive 보강·안전한 선택 본문 전달

2026년 6월 5일 릴리스는 Agent Panel을 실제 업무 입력창처럼 쓰기 위한 파일 첨부 흐름과, 원격 Mac의 OneDrive 클라우드 전용 파일을 더 안정적으로 다루는 데 초점을 맞췄습니다. 긴 선택 본문은 화면에 그대로 붙이지 않고 컨텍스트 파일로 저장해 Claude에 전달하므로, 패널이 과도하게 길어지는 문제도 줄였습니다.

- **Agent Panel 파일 첨부** — Finder/Explorer 또는 파일 탐색기에서 Agent Panel로 파일을 드래그하거나, 파일을 복사한 뒤 입력창에 붙여넣으면 첨부 chip으로 쌓입니다. 전송 전 제거할 수 있고, 전송 후 대화에도 첨부 내역이 표시됩니다.
- **원격 Agent 경로 보정** — 같은 SSH 프로필의 `ssh://` 파일은 Claude가 직접 읽을 수 있는 원격 경로로 바꿔 전달합니다. 로컬 파일을 원격 Agent에 붙인 경우에는 접근 주의 문구를 함께 보냅니다.
- **선택 본문 안전 전달** — 문서 선택 영역을 Claude에 보낼 때 긴 원문을 대화 화면에 노출하지 않고 사건 폴더의 임시 컨텍스트 파일로 저장한 뒤 그 경로를 전달합니다.
- **Agent 진행 상태 개선** — 작업 중 상태를 스피너로 보여주고, Esc로 진행 중 작업이나 로그인 입력을 중단할 수 있습니다. 큐 대기 수는 상태 옆에 간결하게 표시됩니다.
- **터미널 붙여넣기 안정화** — xterm의 bracketed paste 모드를 따라가도록 바꿔, 일반 셸과 Claude 입력창 사이에서 줄바꿈 처리와 중복 붙여넣기를 더 안정적으로 맞췄습니다.
- **원격 OneDrive 목록/검색 보강** — 원격 Mac에서 OneDrive 클라우드 전용 폴더나 파일이 아직 로컬에 내려오지 않은 경우에도 rclone 목록을 병합해 파일 탐색기와 폴더 검색에서 더 잘 보이도록 했습니다.
- **원격 파일 물리화 개선** — dataless 파일을 읽을 때 대상 디렉터리를 보장하고, 이미 내려받힌 파일은 덮어쓰지 않도록 한 번 더 확인해 OneDrive 경합을 줄였습니다.
- **macOS/Windows 배포 반영** — [v0.1.37 릴리스](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.37)는 macOS arm64/x64, Windows x64 빌드와 업데이트 메타데이터를 포함합니다.
