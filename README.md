...
## 이번 업데이트 (v0.1.40) — Markdown 문서 이름 변경·저장 상태 보강

2026년 6월 5일 릴리스는 Markdown 문서 탭과 편집 저장 흐름을 다듬는 데 초점을 맞췄습니다. 문서 탭 이름 변경이 실제 파일 이름과 자연스럽게 이어지고, 새 문서 저장 이름도 현재 탭 제목을 더 잘 반영합니다.

- **Markdown 탭 이름 변경** — Markdown 문서 탭 이름을 바꾸면 연결된 `.md`/`.markdown` 파일 이름도 함께 변경됩니다.
- **확장자 보존** — 확장자 없이 제목만 입력해도 기존 Markdown 확장자를 유지하고, 이미 확장자를 입력한 경우에는 그대로 존중합니다.
- **새 문서 저장 이름 개선** — 아직 경로가 없는 새 문서를 저장할 때 탭 제목을 기본 파일명으로 사용합니다.
- **경로 변경 후 저장 상태 동기화** — 파일 이름 변경이나 다른 이름 저장 뒤 편집기의 내부 경로·저장 상태가 현재 문서와 맞게 갱신됩니다.
- **Markdown 가로줄 프리뷰 안정화** — `---` 같은 가로줄 문법을 줄 장식으로 렌더링해 편집 레이아웃 흔들림을 줄였습니다.
- **macOS/Windows 배포 반영** — [v0.1.40 릴리스](https://github.com/jurisupport/legal-terminal/releases/tag/v0.1.40)는 macOS arm64/x64, Windows x64 빌드와 업데이트 메타데이터를 포함합니다.
